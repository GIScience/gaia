...
category_partitions = StaticPartitionsDefinition(
    ["demographics", "facilities", "ndvi", "crops", "rural", "access", "coping", "vulnerability", "exposure", "rai"]
)
...
def acled_hrp_asset(context, boundary_asset: str) -> str:
    """
    ...
    if not boundary_path:
        context.log.error(f"[{country_code}] No boundary file found for {fallback_levels}")
    ...
    if xlsx_path.exists():
        ...
        context.log.info(f"[{country_code}] Downloading ACLED HRP data from {ACLED_HDX_URL}")
        try:
    ...
    if pcode_col not in df.columns:
        context.log.error(f"Column '{pcode_col}' not found. Available columns: {df.columns.tolist()}")
        raise ValueError(f"Missing column: {pcode_col}")

    country_rows = df[df[pcode_col].isin(adm2_pcodes)]
    context.log.info(f"[{country_code}] Filtered to {len(country_rows)} rows matching country pcodes")
    ...
    for yr in range(2018, 2026):
        ...
        yr_agg = (
            ...
            .agg(**{f"acled_evts_{suffix}": ("Events", "sum"), f"acled_fatl_{suffix}": ("Fatalities", "sum")})
            .reset_index()
    ...
    result["acled_evts_w"] = sum(result[col].fillna(0) * w for col, w in zip(evt_cols, weights))
    result["acled_fatl_w"] = sum(result[col].fillna(0) * w for col, w in zip(fatl_cols, weights))
